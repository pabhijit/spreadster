...
from spreadster.live.models import MarketState, RegimeAssessment

def classify_regime(state: MarketState) -> RegimeAssessment:
    rationale = []
    upper_em = state.previous_close + state.expected_move
    lower_em = state.previous_close - state.expected_move

    if state.open_price > state.previous_close and abs(state.open_price - upper_em) <= 0.20 * state.expected_move:
    ...
    if state.open_price < state.previous_close and abs(state.open_price - lower_em) <= 0.20 * state.expected_move:
